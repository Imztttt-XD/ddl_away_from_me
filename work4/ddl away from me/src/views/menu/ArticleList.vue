...
const loadArticle = async () => {
  try {
    ...
    if(orderType.value===0){
      let result = await getRankingListService()
      if (result.code === 1) {
    ...
    if(orderType.value===1){
      let result = await getCurrentListService()
      if (result.code === 1) {
        articleListInfo.value = result.data
      } else {
        return false;
    ...
    <el-container>
      <el-header>
        <el-menu router mode="horizontal">
          <el-menu-item :class="tabClass(0)"
          @click="changeOrderType(0)">推荐</el-menu-item>
          <el-menu-item :class="tabClass(1)"
          @click="changeOrderType(1)">最新</el-menu-item>
        </el-menu>
      </el-header>
